...
def batch_processor(model, data, train_mode=False, **kwargs):
    ...
    if train_mode:
        ...
        return model(example, is_ema=[False, None], return_loss=False)
...
def train_detector(model, dataset, cfg, distributed=False, validate=False, logger=None):
    ...
    # build trainer
    trainer = Trainer(model, model_ema, batch_processor, optimizer, lr_scheduler, cfg.work_dir, cfg.log_level)
